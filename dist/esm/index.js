...
import React, { useState, useMemo, useRef, useReducer, useEffect, useCallback, forwardRef, useImperativeHandle, Fragment, memo, createContext, useContext } from 'react';
...
var Dropzone = /*#__PURE__*/forwardRef(function (_ref, ref) {
  ...
  return /*#__PURE__*/React.createElement(Fragment, null, children(_objectSpread$1(_objectSpread$1({}, props), {}, {
    open: open
  })));
});
...
const Button = ({ variant, description, width, height, icon, hint, disabled, disabledHover, style, ...props }) => {
    return (jsxs(StyledButton, { "$variant": variant, "$width": width, "$height": height, "$hasDescription": !!description, "$customStyle": style, title: hint, disabled: disabled, "$disabledHover": disabledHover, ...props, children: [icon && jsx(IconWrapper$2, { children: icon }), description && jsx(Description, { children: description })] }));
};
...
const ConfirmModal = ({ isOpen, title, content, onClose, onConfirm, modalWidth = '400px', variantPrimary = 'warning', variantSecondary = 'secondary', confirmLabel = 'ACEITAR', cancelLabel = 'CANCELAR', confirmButtonProps, cancelButtonProps, }) => {
    ...
    return (jsx(Modal, { isOpen: isOpen, variant: variantPrimary, title: title, content: content, modalWidth: modalWidth, maxWidth: "85%", onClose: onClose, showCloseButton: false, actions: jsxs(Fragment$1, { children: [renderButton(cancelLabel, variantSecondary, onClose, cancelButtonProps), renderButton(confirmLabel, variantPrimary, () => {
                    onConfirm();
                    onClose();
                }, confirmButtonProps)] }) }));
};
...
const FieldValue = ({ type, value = '', variant, description, hint, editable = true, width, maxWidth, maxHeight, minValue, maxValue, inputWidth, inline, options, icon, padding, placeholder, maxDecimalPlaces = 2, maxIntegerDigits = 8, onUpdate, onKeyDown, }) => {
    ...
    const renderInput = () => (jsxs(Fragment$1, { children: [icon && jsx(Icon, { children: icon }), jsx(StyledInput, { type: editable ? type : 'string', readOnly: !editable, disabled: !editable, value: displayValue, onChange: handleChange, onKeyDown: onKeyDown, placeholder: placeholder, "$inputWidth": inputWidth, "$inline": inline, "$variant": variant, "$editable": editable })] }));
    const renderSelect = () => (jsxs(StyledSelect, { value: displayValue, onChange: handleChange, disabled: !editable, "$inputWidth": inputWidth, "$inline": inline, "$variant": variant, "$editable": editable, children: [type === 'SELECT' && jsx("option", { value: "", children: placeholder || 'Selecione...' }), type === 'SELECT'
                ...
                : (jsxs(Fragment$1, { children: [jsx("option", { value: "true", children: "Sim" }), jsx("option", { value: "false", children: "N\u00E3o" })] }))] }));
...
const Stack = ({ children, width, height, direction = 'row', divider, style, alignCenter, alignRight, justifyCenter, justifyBetween, gap, wrap }) => {
    return (jsx(StackContainer, { "$direction": direction, "$divider": divider, "$gap": gap, "$width": width, "$height": height, "$alignCenter": alignCenter, "$alignRight": alignRight, "$justifyCenter": justifyCenter, "$justifyBetween": justifyBetween, "$wrap": wrap, style: style, children: children }));
};
...
const SearchFilterRSQL = ({ fields, onSearch, locale = 'pt' }) => {
    ...
    return (jsx(Container$1, { children: jsxs(Stack, { direction: "column", divider: "top", children: [jsxs(FilterFieldsRow, { children: [jsx(FieldValue, { type: "SELECT", value: selectedField?.name || '', options: fields.map(({ name, label }) => ({ key: name, value: label })), onUpdate: handleFieldChange, editable: true, placeholder: text.selectPlaceholder }), jsx(FieldValue, { type: "SELECT", value: selectedOperator?.name || '', options: selectedField
                            ...
                            }, editable: !!selectedField, placeholder: text.selectPlaceholder }), jsx(FieldValue, { type: selectedField?.type || 'STRING', value: searchValue || '', onUpdate: setSearchValue, editable: !!selectedOperator, options: selectedField?.type === 'SELECT' ? selectedField.options : undefined, onKeyDown: (e) => e.key === 'Enter' && handleAdd(), placeholder: text.selectPlaceholder }), jsx(Button, { icon: jsx(FaPlus, {}), onClick: handleAdd, hint: text.addHint, variant: "success", width: "36px", height: "36px", disabled: isAddButtonDisabled(), style: { borderRadius: '0 5px 0 0', flexShrink: 0 } })] }), filters.length > 0 ? (jsx(Tags, { children: filters.map((f, i) => (jsxs(Tag, { children: [jsxs("span", { children: [fields.find(fd => fd.name === f.field)?.label, " ", f.operadorDescr, " ", getFormattedValue(f)] }), jsx(Button, { icon: jsx(FaTimes, {}), onClick: () => handleRemove(i), variant: "warning", height: "20px", width: "20px", style: {
                                    borderRadius: '50%',
                                    justifyContent: 'center',
                                    alignItems: 'center',
                                    display: 'flex'
...
const Tabs = ({ tabs }) => {
    ...
    return (jsxs(Container$1, { width: '100%', backgroundColor: 'transparent', children: [jsx(TabList, { children: tabs.map((tab, index) => (jsx(TabButton, { "$active": index === activeTab, onClick: () => handleTabClick(index), children: tab.label }, index))) }), jsx(TabContent, { children: tabs[activeTab]?.content })] }));
};
...
const HighlightBox = ({ variant = 'info', width = '100%', height = '100%', bordered = false, style, children, }) => {
    return (jsx(CenterWrapper, { children: jsx(Box, { "$variant": variant, "$width": width, "$height": height, "$bordered": bordered, style: style, children: children }) }));
};
...
const Breadcrumb = ({ items, LinkComponent = null }) => {
    if (!items.length)
        return null;
    return (jsx(BreadcrumbContainer, { children: items.map((item, index) => (jsx("span", { children: item.path && LinkComponent && index < items.length - 1 ? (jsx(LinkComponent, { to: item.path, children: item.label })) : (jsx("strong", { children: item.label })) }, item.path ?? item.label))) }));
};
